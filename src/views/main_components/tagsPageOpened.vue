...
export default {
  ...
  methods: {
    ...
    closePage(event, name) {
        let lastName = this.$store.state.lastPageName
      ...
      if (this.currentPageName === name) {
        let lastPageName = "";
        let page = this.$store.state.pageOpenedList.find(
          item => item.name == lastName
        );
        if (page) {
          lastPageName = page.name;
        } else if (this.$store.state.pageOpenedList.length > 1) {
  ...
  created() {
    bus.$on("closePage", this.closePage);
  },
  destroyed() {
